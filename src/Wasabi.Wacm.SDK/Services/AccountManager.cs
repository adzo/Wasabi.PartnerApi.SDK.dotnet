...
namespace Wasabi.Wacm.SDK.Services
{
    internal sealed class AccountManager : IAccountManager
    {
        ...
        public async Task<CreateAccountResponse> CreateAccountAsync(CreateAccountRequest requestObject)
        {
            ...
            var stringContent = new StringContent(jsonObject, Encoding.UTF8, WacmDefaults.JsonContentType);
        ...
        public async Task<Response> DeleteAccountAsync(int accountNumber)
        {
            ...
            //else
            //{
            //    var wcmResponse = JsonSerializer.Deserialize<WacmResponseMessage>(await response.Content.ReadAsStringAsync());
